...
private[trans] sealed trait ReaderPInteract[E, P[+_, -_, -_, +_, +_]] extends Interact[({ type p[+uO, -uI, -dI, +dO, +a] = ReaderP[E, P, uO, uI, dI, dO, a] })#p] with ReaderPProxy[E, P] {
  ...
  def requestWith[A1, A2, K1, K2, B1, B2, C1, C2](p1: B1 => ReaderP[E, P, A1, A2, K1, K2, B2], p2: C1 => ReaderP[E, P, B1, B2, K1, K2, C2]): C1 => ReaderP[E, P, A1, A2, K1, K2, C2] = { v => ReaderP { x => P.requestWith[A1, A2, K1, K2, B1, B2, C1, C2](p1 andThen { _.run(x) }, p2 andThen { _.run(x) })(v) } }

  def respondWith[K1, K2, B1, B2, A1, A2, C1, C2](p1: A1 => ReaderP[E, P, K1, K2, B1, B2, A2], p2: B2 => ReaderP[E, P, K1, K2, C1, C2, B1]): A1 => ReaderP[E, P, K1, K2, C1, C2, A2] = { v => ReaderP { x => P.respondWith[K1, K2, B1, B2, A1, A2, C1, C2](p1 andThen { _.run(x) }, p2 andThen { _.run(x) })(v) } }
